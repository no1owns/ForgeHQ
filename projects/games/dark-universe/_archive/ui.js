...
const UI = (() => {
  ...
  let _pwImg = null;
  let _pwReady = false;
  let _pwSpritePosterImg = null;
  let _pwSpritePosterReady = false;

  function loadPennywisePosterSheet() {
    if (_pwSpritePosterImg) return;
    _pwSpritePosterImg = new Image();
    _pwSpritePosterImg.onload = () => { _pwSpritePosterReady = true; };
    _pwSpritePosterImg.onerror = () => { _pwSpritePosterReady = false; };
    _pwSpritePosterImg.src = 'assets/pennywise-sheet.png?v=12';
  }

  function drawPennywisePosterFrame(canvasEl) {
    if (!_pwSpritePosterReady || !_pwSpritePosterImg || !canvasEl) return false;
    ...
    const frame = { x: 0, y: 38, w: 191, h: 187, anchorX: 95.5, anchorY: 179 };
    const baseScale = Math.min(W / frame.w, H / frame.h) * 0.68;
    const drawW = frame.w * baseScale;
    const drawH = frame.h * baseScale;

    const screenAnchorX = W * 0.5;
    const screenAnchorY = H - 18;
    const dx = screenAnchorX - frame.anchorX * baseScale;
    const dy = screenAnchorY - frame.anchorY * baseScale;

    ctx.drawImage(_pwSpritePosterImg, frame.x, frame.y, frame.w, frame.h, dx, dy, drawW, drawH);
    return true;
  }
  ...
  function buildPennywiseSVG(wrap, phase) {
    ...
    // Draw a static poster frame immediately so Pennywise is visible during intro dialogue
    loadPennywisePosterSheet();
    if (_pwSpritePosterReady) {
      drawPennywisePosterFrame(spriteCanvas);
    } else if (_pwSpritePosterImg) {
      const once = () => {
        drawPennywisePosterFrame(spriteCanvas);
        _pwSpritePosterImg.removeEventListener('load', once);
      };
      _pwSpritePosterImg.addEventListener('load', once, { once: true });
    }
