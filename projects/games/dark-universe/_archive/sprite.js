...
window.Sprite = (() => {
  const LAYOUT = {
  "sheet": "assets/pennywise-sheet.png",
  "meta": {
  ...
  let sharedImg = null;
  let sharedImgReady = false;
  let sharedImgLoading = false;
  let sharedImgCallbacks = [];

  function warmup() {
    if (sharedImgReady || sharedImgLoading) return;
    sharedImgLoading = true;
    sharedImg = new Image();
    sharedImg.onload = () => {
      sharedImgReady = true;
      sharedImgLoading = false;
      const callbacks = sharedImgCallbacks.slice();
      sharedImgCallbacks = [];
      callbacks.forEach(cb => { try { cb(sharedImg); } catch (e) {} });
    };
    sharedImg.onerror = () => {
      sharedImgLoading = false;
      console.warn('[Sprite] Failed to preload sprite sheet');
    };
    sharedImg.src = LAYOUT.sheet + '?v=12';
  }

  function whenImageReady(cb) {
    if (sharedImgReady && sharedImg) {
      cb(sharedImg);
      return;
    }
    sharedImgCallbacks.push(cb);
    warmup();
  }
  ...
  function getState() {
    // Auto-start sprite sheet preload
  warmup();
